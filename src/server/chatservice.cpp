...
MsgHandler ChatService::getHandler(int msgid)
{
    ...
    {
        // 返回一个默认处理器，空操作
        return [=](const net::TcpConnectionPtr &conn, json &js, Timestamp)
        {
...
// 注销业务
void ChatService::loginout(const net::TcpConnectionPtr &conn, json &js, Timestamp time)
{
...
// 一对一聊天业务
void ChatService::oneChat(const net::TcpConnectionPtr &conn, json &js, Timestamp time)
{
...

    {
        ...
        if (it != _userConnMap.end())
        {
            // toid在线，转发消息（服务器主动推送消息至toid用户)
            it->second->send(js.dump());
            return;
        }
    }

    // 查询toid是否在线（可能离线，也可能登录其他server）
    User user = _userModel.query(toid);
    if (user.getState() == "online")
    {
        // toid登录其他server，在redis中向toid订阅的通道发布消息
...
void ChatService::groupChat(const net::TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    for (int id : useridVec)
    {
        ...
        {
            // 查询id是否在线（可能离线，也可能登录其他server）
            User user = _userModel.query(id);
            if (user.getState() == "online")
            {
                // id登录其他server，在redis中向id订阅的通道发布消息
